...
if (isset($_GET['contestid'])) {
    // Retrieve the contestid from the URL
    $contestid = $_GET['contestid'];
    
    $_SESSION['conid']=$contestid;

    echo '<div class="sidebar">
        <a style="text-align:center" href="">Contest Area</a>
        <a style="text-align:center" href="contestrank.php?contestid=' . $contestid .'">Rank</a>
        ...
        echo '<table class="contest-table">
        <thead>
            <tr>
                <th>ID</th>
                <th>Title</th>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Split the problemid into an array
            $problemIds = explode(',', $row['problemid']);

            // Iterate through the problem ids and display the corresponding details
            foreach ($problemIds as $problemId) {
                // Assuming $conn is your database connection
                if($problemId!=''){
                $problemSql = "SELECT * FROM `problemset` WHERE problemid = $problemId";
                ...
                if ($problemResult->num_rows > 0) {
                    while ($problemRow = $problemResult->fetch_assoc()) {
                        ...
                        $color = 'green';
                        if ($problemRow['difficulty'] == "Medium") {
                            $color = 'yellow';
                        } else if ($problemRow['difficulty'] == "Hard") {
                            $color = 'red';
                        }

                        echo '<tr>
                        <td>' . $problemRow['problemid'] . '</td>
                        <td style="width: 50%;">
                            <a href="conteststatement.php?problemid=' . $problemRow['problemid'] . '">' . $problemRow['title'] . '</a>
                        </td>
                        <td style="color:' . $color . '" class="status-running">' . $problemRow['difficulty'] . '</td>
                      </tr>';
